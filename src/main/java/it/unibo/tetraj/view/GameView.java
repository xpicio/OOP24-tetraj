...
/**
 * The game view responsible for rendering the game state. This is the "View" in the MVC pattern.
 */
public final class GameView {
  ...
  /**
   * Initializes the buffer strategy. Must be called after the canvas is added to a visible window.
   */
  ...
  private void renderGame(final Graphics2D g, final GameModel model) {
    ...
    g.fillRect(BOARD_X, BOARD_Y, board.getWidth() * CELL_SIZE, board.getHeight() * CELL_SIZE);
    ...
    for (int row = 0; row < board.getHeight(); row++) {
      for (int col = 0; col < board.getWidth(); col++) {
        ...
        if (color != null) {
          drawCell(g, BOARD_X + col * CELL_SIZE, BOARD_Y + row * CELL_SIZE, color);
        }
      }
    }
  ...
  private void drawTetromino(
      final Graphics2D g,
      final AbstractTetromino<?> tetromino,
      final int boardX,
      final int boardY,
  ...
  private void renderSidePanel(final Graphics2D g, final GameModel model) {
    ...
    final AbstractTetromino<?> next = model.getNextPiece();
    if (next != null) {
    ...
    final AbstractTetromino<?> held = model.getHeldPiece();
    if (held != null) {
    ...
    for (int row = 0; row < shape.length; row++) {
      for (int col = 0; col < shape[row].length; col++) {
        if (shape[row][col] != 0) {
          ...
          g.setColor(color.darker());
          g.drawLine(
              cellX + PREVIEW_CELL_SIZE - 2,
              cellY + 1,
              cellX + PREVIEW_CELL_SIZE - 2,
              cellY + PREVIEW_CELL_SIZE - 2);
          g.drawLine(
              cellX + 1,
              cellY + PREVIEW_CELL_SIZE - 2,
              cellX + PREVIEW_CELL_SIZE - 2,
