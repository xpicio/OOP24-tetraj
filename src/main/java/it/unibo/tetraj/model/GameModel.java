...
import it.unibo.tetraj.model.pieces.TetrominoFactory;
import it.unibo.tetraj.model.pieces.selection.BagRandomizerStrategy;
import java.util.List;

/**
 * The main game model that manages game state and logic. This is the "Model" in the MVC pattern.
 */
public final class GameModel {
  ...
  public void startNewGame() {
    ...
    currentPiece = this.tetrominoFactory.create();
    centerPieceToTheTopCenter(currentPiece);
    nextPiece = this.tetrominoFactory.create();
    centerPieceToTheTopCenter(nextPiece);
  ...
  private void placePiece() {
    ...
    currentPiece = nextPiece;
    nextPiece = this.tetrominoFactory.create();
    nextPiece.setPosition((board.getWidth() - nextPiece.getWidth()) / 2, 0);
    canHold = true;
    fallTimer = 0;
  ...
  public AbstractTetromino<?> getCurrentPiece() {
    return currentPiece == null ? null : currentPiece.copy();
  }
  ...
  public AbstractTetromino<?> getNextPiece() {
    return nextPiece == null ? null : nextPiece.copy();
  }
  ...
  public AbstractTetromino<?> getHeldPiece() {
    return heldPiece == null ? null : heldPiece.copy();
  }
  ...
  private void centerPieceToTheTopCenter(final AbstractTetromino<?> tetromino) {
